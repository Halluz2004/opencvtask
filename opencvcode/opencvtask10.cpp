...
void connected_component_stats(Mat& image) {
    ...
    morphologyEx(binary, binary, MORPH_DILATE, k);//膨胀 填补目标中的空洞
    imshow("形态学操作后", binary);
    ...
    for (int i = 1; i < num_labels; i++) {
        ...
        //相当于  Vec2d pt;   pt[0] = centroids.at<double>(i, 0);   pt[1] = centroids.at<double>(i, 1);
